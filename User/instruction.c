...
void instruction_handle(void)
{
    ...
    if (flag_get_all_status)
    {
        ...
        if (synchronous_request_status == SYN_REQUEST_STATUS_NONE)
        {
            ...
            send_data(SEND_ENGINE_SPEED, fun_info.engine_speeed);              // 发送发动机转速
            send_data(SEND_SPEED, fun_info.speed);                             // 发送当前采集到的车速（时速）
            send_data(SEND_SPEED_WITH_MILE, (u32)fun_info.speed * 621 / 1000); // 发送时速（单位：英里每小时 mile/h） 1km/h == 0.621427mile/h
            send_data(SEND_FUEL, fun_info.fuel);                               // 9. 发送当前油量(单位：百分比)
 
            send_data(SEND_TOTAL_MILEAGE_TENTH_OF_KM, (u32)fun_info.save_info.total_mileage / 100);      // 发送大计里程， 数据 3 byte ， 单位： 0.1 km
            send_data(SEND_SUBTOTAL_MILEAGE_TENTH_OF_MILE, (u32)fun_info.save_info.total_mileage / 161); // 发送大计里程， 数据 3 byte ， 单位： 0.1 mile 

            send_data(SEND_TOTAL_MILEAGE_WITH_MILE, fun_info.save_info.total_mileage / 1610);               // 发送大计里程（总里程，单位：英里 mile），只发送1英里以上的数据
            send_data(SEND_SUBTOTAL_MILEAGE_WITH_TENTH_OF_MILE, fun_info.save_info.subtotal_mileage / 161); // 发送小计里程（总里程，单位：0.1 英里 0.1 mile），只发送0.1英里以上的数据
            ...
            send_data(SEND_SUBTOTAL_MILEAGE_2_WITH_TENTH_OF_MILE, fun_info.save_info.subtotal_mileage_2 / 161); // 发送小计里程2(只发送0.1英里及以上的数据)
        }
    }
    ...
    if (flag_get_speed)
    {
        ...
        send_data(SEND_SPEED_WITH_MILE, (u32)fun_info.speed * 621 / 1000);
    }
    ...
    if (flag_get_total_mileage) // 如果要获取大计里程 / 得到了大计里程新的数据
    {
        ...
        send_data(SEND_TOTAL_MILEAGE_WITH_MILE, fun_info.save_info.total_mileage / 1610); // 1km == 0.621427mile
    ...
    if (flag_get_sub_total_mileage) // 如果要获取小计里程 / 得到了小计里程新的数据
    {
        ...
        send_data(SEND_SUBTOTAL_MILEAGE_WITH_TENTH_OF_MILE, fun_info.save_info.subtotal_mileage / 161);
    ...
    if (flag_get_sub_total_mileage_2) // // 如果要获取小计里程2 / 得到了小计里程2新的数据
    {
        ...
        // 1km == 0.621427mile，1km == 6.21427 * 0.1 mile
        send_data(SEND_SUBTOTAL_MILEAGE_2_WITH_TENTH_OF_MILE, fun_info.save_info.subtotal_mileage_2 / 161);
    }
...
// printf("handle update_date \n");
    ...
    if (flag_get_all_status)
    {
        ...
        if (synchronous_request_status == SYN_REQUEST_STATUS_NONE)
        {
            ...
            // send_data_packet(SEND_SUBTOTAL_MILEAGE_TENTH_OF_MILE); // 发送大计里程， 数据 3 byte ， 单位： 0.1 mile

            // send_data_packet(SEND_TOTAL_MILEAGE_WITH_MILE);             // 发送大计里程（总里程，单位：英里 mile），只发送1英里以上的数据
            // send_data_packet(SEND_SUBTOTAL_MILEAGE_WITH_TENTH_OF_MILE); // 发送小计里程（总里程，单位：0.1 英里 0.1 mile），只发送0.1英里以上的数据
